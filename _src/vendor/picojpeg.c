...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
/* XNET: the OV519 EyeToy's MJPEG delivers its two chroma components in the
   opposite order to what picojpeg's fixed block dispatch assumes, so Cb and Cr
   come out swapped (red->purple, blue->orange, yellow->blue, green correct).
   These macros swap the chroma conversion across every scan-type path. Flip
   XNET_CHROMA_SWAP to 0 to restore stock picojpeg behavior. */
#define XNET_CHROMA_SWAP 1
#if XNET_CHROMA_SWAP
#  define CB_CONV  convertCr
#  define CR_CONV  convertCb
#  define CB_UPV   upsampleCrV
#  define CR_UPV   upsampleCbV
#  define CB_UPH   upsampleCrH
#  define CR_UPH   upsampleCbH
#  define CB_UP    upsampleCr
#  define CR_UP    upsampleCb
#else
#  define CB_CONV  convertCb
#  define CR_CONV  convertCr
#  define CB_UPV   upsampleCbV
#  define CR_UPV   upsampleCrV
#  define CB_UPH   upsampleCbH
#  define CR_UPH   upsampleCrH
#  define CB_UP    upsampleCb
#  define CR_UP    upsampleCr
#endif
